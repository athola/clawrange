...
3. **Ship a working reference profile**: the lead/CRM bot: scheduled
   ...
   natural-language querying of that CRM (relational and time-series) on
   ...
   from the profile and `.env`, so it works against export-link, API-export,
...
- **Bash scripts portable** (macOS and Ubuntu). `:latest` images for testing.
- **New dependency budget**: `pyyaml` (already present at 6.0.1, needs
  pinning). Avoid heavyweight deps. Prefer stdlib and httpx (already pinned).

### Resources
- **Timeline**: This branch (directive: ignore scope guard, stay on branch).
- **Team**: Solo and Claude Code, TDD discipline.
...
- [ ] The scrape source authenticates via profile-declared auth params
      ...
      `.env`, and unit-tested against a local fixture server for at least the
...
- [ ] A new profile can be added by dropping a YAML file and (optional)
...
### Approach A: Profile-as-data and connector registry
...
**Pros**: Unlimited per-tenant power, and natural for developers.

**Cons**: Defeats "business owner, not a Python dev" goal, heavier
coupling, harder to sandbox, and bigger blast radius.
...
### Approach C: Hybrid declarative YAML profile and code connector registry ⭐
...
**Stack**: pyyaml, APScheduler (existing), SQLite (CRM and brain), httpx
(scrape and REST), llm_proxy (NL query router).

**Pros**:
- Clean separation: operators compose, developers extend.
- "Business owner configures without Python" for the common path, and full
...
**Cons**:
- Two layers to learn (profile YAML and primitive registry).
...
## Selected Approach C: Hybrid declarative profile and connector registry ⭐

### Rationale

The user's four design selections collectively *are* a vote for C:
**full template extraction** (persona and seeds out of code), a **local
SQLite CRM** reference (testable primitive in the registry),
**authenticated session scrape** (a source primitive), and **templated
queries and LLM router** (a code-level query layer composed via profile
...
### Trade-offs Accepted
- **Two-layer model (YAML and registry)** → Mitigation: a single documented
  profile schema and a `make profile` render/seed command. The registry is
...
2. `Skill(attune:project-planning)`: architecture and dependency-ordered tasks

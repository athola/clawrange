...
- `lead-crm`, the reference business profile: scheduled authenticated
  scrape of leads → clean → load into a pluggable CRM (SQLite default) and
...
```
config/profiles/
  marketing/profile.yaml      # extracted John-117, 5 projects, and 3 schedules
...
- **Acceptance**:
  - Loading `marketing` returns a profile whose `seeds.projects` equals the
    ...
    normalized dicts).
  - Loading a profile with `${PORTAL_TOKEN}` set resolves it. Unset → "" and warning.
...
- **FR-4.4** `get_adapter(crm_cfg)` returns the adapter named by
  `crm_cfg["adapter"]`.
- **Acceptance**:
  - `SQLiteCRM.upsert` inserts new and updates existing on `upsert_key`,
...
- **FR-5.2** `route_nl(prompt, templates, *, llm=_llm_call)` asks the LLM to
  choose a template name and params as JSON, given the template
...
- **Acceptance**:
  ...
  - `crm_digest` formats `new_leads_count` and `leads_by_status` into a digest
    string and calls `telegram.notify`.

### FR-7: HTTP API (`workflows/app.py` and CRM router)
...
### FR-8: Profile-driven lifespan and extraction (`workflows/app.py`)
...
- **Acceptance**: with `CLAWRANGE_PROFILE` unset, boot seeds the five
  marketing projects and three schedules exactly as before (existing
...
## Test Strategy
- **Unit**: `test_profile.py` (load/validate/env and seed-equivalence),
  `test_persona.py` (render and genericity guard), `test_connectors.py`
...
- **Fixtures**: a local `pytest` fixture HTTP server (stdlib `http.server` or
  httpx `MockTransport`) for http_csv and login_scrape. Seeded `crm.db` for
  query tests.

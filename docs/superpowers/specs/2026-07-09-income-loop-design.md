...
| "Research technique X" | `research:tome: <topic>` task and tome bridge (`research_pulse` pattern) |
| Weekly reevaluation | New `income_review` generator → `[DRAFT]` task and Telegram |
| Approval gate | Existing task queue claim/result and `[DRAFT]` convention |
...
### Phase 3: deferred (needs Phase 1 and 4 weeks of history)
